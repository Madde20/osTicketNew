...
<div id="basic_search">
    <div class="well">
        <div class="form-inline">
            <div class="form-group dashboard-search">
        <!--<p><?php //echo __('Select the starting time and period for the system activity graph');?></p>-->
            <?php echo csrf_token(); ?>
            <label>
                <?php echo __( 'Report timeframe'); ?>:</label>
                <input type="text" class="dp input-medium search-query form-control"
            ...
            <button class="green button action-button muted btn btn-default leftten" type="submit">
                <?php echo __( 'Refresh');?>
            </button>
...
<div style="position:relative">
    <div id="line-chart-here" style="height:300px; width:100%"></div>
...
foreach ($groups as $g=>$desc) { ?>
    <li class="<?php echo $first ? 'active' : ''; ?>"><a data-toggle="tab" href="#<?php echo Format::slugify($g); ?>"
...
foreach ($groups as $g=>$desc) {
    ...
    <div class="tab-pane fade <?php echo (!$first) ? '' : 'in active'; ?>" id="<?php echo Format::slugify($g); ?>">
    <table class="table table-bordered dashboard-stats"><tbody><tr>
<?php
